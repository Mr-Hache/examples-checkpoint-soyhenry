...
describe("<TeamDetail />", () => {
  ...
  const store = (id) => {
    let state = {
      ...
      teamDetail: id !== 6 ? data.teams[id - 1] : data.teams.concat(noTeam),
    };
  ...
  beforeEach(() => {
    ...
    useSelectorFn = (id) =>
      useSelectorStub.mockReturnValue(store(id).getState().teamDetail);
  ...
  it("Debería usar un useEffect y dentro de este, dispachar la acción getTeam, pasandole como argumento el ID del team a renderizar", () => {
    ...
    const getTeamDetail = jest.spyOn(actions, "getTeamDetail");
    teamDetail(1);
    expect(useEffect).toHaveBeenCalled();
    expect(useDispatch).toHaveBeenCalled();
    expect(getTeamDetail).toHaveBeenCalled();
  });
