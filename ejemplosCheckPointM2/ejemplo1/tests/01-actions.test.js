...
describe("Actions", () => {
  ...
  describe("getTeamDetail", () => {
    it('Debería hacer un dispatch con las propiedades type "GET_TEAM_DETAIL" y como payload, el resultado del fetch al link provisto', async () => {
      const payload = data.teams[0];
      return store
        .dispatch(getTeamDetail(payload.id))
        .then(() => {
          const actions = store.getActions();
          // eslint-disable-next-line jest/no-conditional-expect
