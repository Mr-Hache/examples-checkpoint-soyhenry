...
BinarySearchTree.prototype.getHouseValues = function (
  side,
  total = 0,
  band = 0,
  primerValor = 0
  ...
  total += this.value;
  if (band === 0) {
    primerValor = this.value;
    band = 1;
  }
  if (side === "left") {
    if (this.left) {
      return this.left.getHouseValues(side, total, band, primerValor);
    }
  }
  if (side === "right") {
    if (this.right) {
      return this.right.getHouseValues(side, total, band, primerValor);
    }
  }

  return total - primerValor;
};
