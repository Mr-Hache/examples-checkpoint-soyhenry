...
function construccionCasas(bolsas) {
    ...
    return function(casas){
        ...
        let bolsasPorCasas = 10;

        if(bolsas < bolsasPorCasas){
            return "No se puede construir casas con esa cantidad de bolsas";
        }

        if(casas > bolsasPorCasas){
            return `Solo puedes construir esta cantidad de casas: ${Math.floor(bolsas / bolsasPorCasas)}`
        }

        return bolsas % bolsasPorCasas === 0
    }
}
